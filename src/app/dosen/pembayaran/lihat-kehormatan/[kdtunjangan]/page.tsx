...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        router.push(`/dosen/pembayaran/lihat-kehormatan/${formData.kdtunjangan}`)
      } else {
  ...
  const filteredSalaryData = salaryData.filter(
    (item) => item.nmpeg.toLowerCase().includes(searchQuery.toLowerCase()) || item.nip.includes(searchQuery),
  )
  ...
  const totalPages = Math.ceil(filteredSalaryData.length / itemsPerPage) || 1
  const indexOfLastItem = currentPage * itemsPerPage
  const indexOfFirstItem = indexOfLastItem - itemsPerPage
  const currentItems = filteredSalaryData.slice(indexOfFirstItem, indexOfLastItem)
    ...
    <section className="text-black px-4 md:px-16">
      ...
      <div className="max-w-6xl mx-auto bg-white p-4 md:p-6 rounded-lg shadow-lg mt-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
          <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-0">LIHAT TUNJANGAN KEHORMATAN</h2>
        </div>
              ...
              {listTunjangan.map((item, index) => (
                <tr key={index} className="border">
                  ...
                  <td className="px-6 border border-gray-300 py-4 text-center">
                    {item.besartunjangan.toLocaleString()}
                  </td>
                  <td className="px-6 border border-gray-300 py-4 text-center">{item.totalpph.toLocaleString()}</td>
                  <td className="px-6 border border-gray-300 py-4 text-center">{item.jmlditerima.toLocaleString()}</td>
                  <td className="px-6 border border-gray-300 py-4 text-center">
