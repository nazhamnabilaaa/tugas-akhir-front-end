...
export default function Page() {
  ...
  const filteredData = tanggalkehormatanList.filter((item) =>
        ...
        <section className="flex-1 text-black px-16">
          <div className="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-lg mt-10">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">TUNJANGAN KEHORMATAN</h2>
            </div>
            <div className="flex justify-between items-center mb-4 mt-8">
              <div className="flex items-center space-x-2">
                ...
                <select id="entries" className="border rounded p-2 w-32" value={entries} onChange={(e) => setEntries(Number(e.target.value))}>
              ...
              <div className="flex space-x-2">
                <Link href="/admin/pembayaran/tambah-kehormatan" className="flex flex-col items-center cursor-pointer">
            ...
            <table className="min-w-full bg-white border-collapse border-black border">
              ...
              <tbody>
                {paginatedData.length > 0 ? (
                    ...
                    <tr key={index} className="border">
                      ...
                      <td className="px-6 border border-gray-300 py-4 text-center">
                        <div className="flex justify-center space-x-2">
                          <Link href={`/admin/pembayaran/lihat-kehormatan/${item.kdtunjangan}`}>
                            <MdPayment className="text-[#2552F4] text-lg" />
                          </Link>
                          <TbTrash className="text-[#E20202] text-lg cursor-pointer" onClick={() => handleDelete(item.kdtunjangan)} />
                        </div>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-4">Tidak ada data ditemukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
